...
def kL_tot_poly(C, *coeffs, Mfunc, Rfunc, propA, propB, elaborate = False):
    '''
...
def kL_tot_muggianu(c : list, eps_list, Mfunc, Rfunc, prop_list : list):
    gamma_quat, gammaM_quat, gammaS_quat = muggianu_model_gamma(c, eps_list, Mfunc, Rfunc, prop_list)
    kL = kL_from_gamma_multi(gamma_quat, prop_list, c)
    return kL, gamma_quat, gammaM_quat, gammaS_quat



def fit_eps_poly(Mfunc, Rfunc, data, prop_list, n_coeffs, p0 = [0,0,0]):
    coeffs, cov = curve_fit(lambda c, *eps_list:\
                         kL_tot_poly(c, eps_list, Mfunc, Rfunc, prop_list), data[0],\
...
#        for y in np.arange(first, 1, (last- first)/n):
#            c_list = np.array([x * y, x * (1-y) , (1-x) * y]) 
#            kL_full[j,i], gamma_full[j,i], gammaM_full[j,i], gammaV_full[j,i]  = kL_tot_quat_muggianu(c_list, eps_list, Mfunc, Rfunc, propA, propB)
#            j = j+1
#        i = i+1
#    return kL_full, gamma_full, gammaM_full, gammaV_full
#
#def run_kL_quat_data_dict_poly(Mfunc, Rfunc, coeffs_list, n, propA, propB : list):
#    '''
#    x : concentration of Sn
#    y : concentration of Se?
#    '''
...
#        for y in np.arange(first, 1, (last- first)/n):
#            c_list = np.array([x * y, x * (1-y) , (1-x) * y]) 
#            kL_full[j,i], gamma_full[j,i], gammaM_full[j,i], gammaV_full[j,i]  = kL_tot_quat_muggianu_poly(c_list, coeffs_list, Mfunc, Rfunc, propA, propB)
#            j = j+1
#        i = i+1
#    return kL_full, gamma_full, gammaM_full, gammaV_full
